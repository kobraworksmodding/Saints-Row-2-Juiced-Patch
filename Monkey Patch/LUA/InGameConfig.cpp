...
namespace InGameConfig {
    ...
    { "ImprovedNPCDespawnRange",&Render3D::CMPatches_CIncreaseNPCDespawnDistance , nullptr,"Gameplay","ImprovedNPCDespawnRange"}
    };
    void AddOptions() {
        ...
        InGameConfig::RegisterBoolSlider("ImprovedNPCDespawnRange", "J_PEDFADE");
        ...
        if (strcmp(var, "IVRadarScaling") == 0) {
            ...
            *value = std::clamp(Behavior::sticky_cam_timer_add / 500, 0, 20);
                ...
                if (Behavior::sticky_cam_timer_add != 0)
                    (void)Behavior::cf_do_control_mode_sticky_MIDASMHOOK.enable();
                else (void)Behavior::cf_do_control_mode_sticky_MIDASMHOOK.disable();
            }
        }
        if (strcmp(var, "X360GammaUI") == 0) {
            if (!write) {
                *value = Render2D::final_2d_render.enabled();
                ...
                if (*value)
                    (void)Render2D::final_2d_render.enable();
                else
                    (void)Render2D::final_2d_render.disable();
