...
namespace MStrings
{
	...
	static wchar_t* AllocLocalizationString(uint32_t hash, const std::wstring& text)
	{
		...
		// This is important:
		// text_ptr - 4 should be the live localization hash.
	...
	static bool ParseCxtFile(const std::string& filepath, std::vector<ParsedString>& out)
	{
		...
		while (std::getline(file, line))
		{
			...
			// Empty lines are ignored unless they are inside text.
			if (trimmed.empty())
				continue;
	...
	void StartParsing()
	{
		...
		for (auto& entry : DirCache)
		{
			...
			for (auto& s : parsed)
			{
				// Allocate into Localization_static_mempool style memory.
				wchar_t* text_ptr = AllocLocalizationString(s.Hash, s.Text);

				if (!text_ptr)
					continue;

				// Store this in your lookup map.
				// Example:
