...
#define ARROW_LEFT (bool*)0x02349504
	...
	struct PC_port_key_assignments
	{
		int button;
		int pad_button;
	};
	PC_port_key_assignments* PC_port_key_for_controler_assignments = (PC_port_key_assignments*)0x234E768;
	...
	wchar_t* __cdecl getpckeyboardimage_hook(unsigned int* actions, int mouse) {
		if (LastInputUI() == GAME_LAST_INPUT::CONTROLLER) {
			...
			if (!actions) {
				if (mouse == 1) {
					...
					return getpckeyboardimage_T.ccall<wchar_t*>(actions, mouse);
			}
			int pad_button = PC_port_key_for_controler_assignments[*actions + 2].pad_button;
			int button = PC_port_key_for_controler_assignments[*actions + 2].button;

			auto it = padButtonMaps.find(pad_button);
			if (it != padButtonMaps.end()) {
				const wchar_t* buttonImage = usePS3Prompts ? it->second.ps3 : it->second.xbox;
				wsprintf(&prompt_image_buffer[prompt_image_buffer_index], L"[format][scale:1.0][image:%s][/format]", buttonImage);
			}
			else if (pad_button == -1) {
				auto& actionsMap = usePS3Prompts ? actionsToPS3 : actionsToXbox;
				auto actionfinder = actionsMap.find(*actions);
				if (actionfinder != actionsMap.end())
					...
					wsprintf(&prompt_image_buffer[prompt_image_buffer_index], L"[format][scale:0.7]Action %d[/format]", *actions);
				}
			}
			else {
				wsprintf(&prompt_image_buffer[prompt_image_buffer_index], L"[format][scale:0.7]Pad %d[/format]", pad_button + 1);
				Logger::TypedLog("ERROR", "Was supposed to give an item out! pad_button %d actions %d button %d ,mouse %d\n", pad_button, *actions, button, mouse);
			}
			prompt_image_buffer_index += wcslen(&prompt_image_buffer[start_index]) + 1;
			return &prompt_image_buffer[start_index];
		}
	...
	wchar_t* __cdecl pc_get_action_pad_pure_text_hook(int action_index) {
		...
		int pad_button = PC_port_key_for_controler_assignments[action_index + 2].pad_button; // No +1
